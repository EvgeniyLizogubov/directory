...
import com.github.evgenylizogubov.directory.error.ErrorType;
...
import static com.github.evgenylizogubov.directory.error.ErrorType.*;

@RestControllerAdvice
@AllArgsConstructor
@Slf4j
public class RestExceptionHandler {
    public static final String ERR_PRX = "ERR# ";
    ...
    private ProblemDetail processException(@NotNull Exception ex, HttpServletRequest request, Map<String, Object> additionalParams) {
        ...
        if (optType.isPresent()) {
            log.error(ERR_PRX + "Exception {} at request {}", ex, path);
            return createProblemDetail(ex, optType.get(), ex.getMessage(), additionalParams);
        } else {
            Throwable root = getRootCause(ex);
            log.error(ERR_PRX + "Exception " + root + " at request " + path, root);
            return createProblemDetail(ex, APP_ERROR, "Exception " + root.getClass().getSimpleName(), additionalParams);
        }
    }

    private ProblemDetail createProblemDetail(Exception ex, ErrorType type, String defaultDetail, @NotNull Map<String, Object> additionalParams) {
